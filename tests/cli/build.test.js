...
describe("CLI build command for all apps", () => {
  getAppNames().forEach(app => {
    it(`should build app '${app}' without error`, async () => {
      const { stderr, exitCode } = await execa("node", [CLI_PATH, "build", app], { reject: false });
