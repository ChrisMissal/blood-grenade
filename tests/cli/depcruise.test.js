...
describe("CLI depcruise command for all apps", () => {
  getAppNames().forEach(app => {
    it(`should depcruise app '${app}' without error`, async () => {
      const { stderr, exitCode } = await execa("node", [CLI_PATH, "depcruise", app], { reject: false });
