...
function printTable(results: InspectionResult[]) {
  for (const result of results) {
    ...
    console.log("Name | Type | ARB | Runtime | Build | Status | Taxonomy | Stereotype | 3rd Party | Hosts | Path");
    console.log("--- | --- | --- | --- | --- | --- | --- | --- | --- | --- | ---");
    ...
    for (const application of result.detectedApplications) {
      ...
      const thirdParty = application.thirdPartyIntegrations
        ...
        .join(", ");
      const hosts = String(application.externalHostDependencies.length);
      console.log(
        `${application.name} | ${application.type} | ${application.arbCategory} | ${application.languageRuntimeGuess} | ${application.buildSystemGuess} | ${application.statusHint} | ${taxonomy || "n/a"} | ${stereotypes || "n/a"} | ${thirdParty || "n/a"} | ${hosts} | ${application.rootPath}`,
      );
    }
  }
}
